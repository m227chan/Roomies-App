...
app.post('/api/getOwedSummary', (req, res) => {
	...
	connection.query(getOwedSummarySQL, getOwedSummary, (error, results, fields) => { 
		if (error) {
			console.log(error.message);
		}
...
app.post('/api/getOwedByPerson', (req, res) => {
	...
	connection.query(getOwedPersonSQL, getOwedPersonData, (error, results, fields) => { 
		if (error) {
			console.log(error.message);
		}
...
app.post('/api/getExpenseReport', (req, res) => {
	...
	let connection = mysql.createConnection(config);
	//Input: (Firebase ID, Sort Value (4 Character String see below), Filter)
	//Output: Table of Transaction related to the user.
	let getExpensesSQL = `
	SET @roomie := (Select id from zzammit.Roomate where firebaseUID = (?)); 
	SET @orderV := (?);
	...
	CREATE TEMPORARY TABLE zzammit.ExpLog Select Expenses.id as ExpenseID, CONCAT(firstName, ' ', lastName) as Spender, idDebtor, amount, tag, comments, tdate 
		From zzammit.Expenses left join zzammit.Roomate on Expenses.idSpender = Roomate.id where (idSpender = @roomie or idDebtor = @roomie);
	...
	DROP TABLE IF EXISTS zzammit.ExpLog;
	`;
	let getExpensesData = [req.body.user, req.body.sort, req.body.filter];

	// console.log(req.body);

	connection.query(getExpensesSQL, getExpensesData, (error, results, fields) => { 
		if (error) {
			console.log(error.message);
		}
...
app.post('/api/addExpense', (req, res) => {
	...
	`;
	let addExpenseData = [req.body.amount, req.body.spender, req.body.debtor, req.body.tag, req.body.comment, req.body.date];
...
app.post('/api/editExpense', (req, res) => {
	...
	Update zzammit.Roomate set owed = owed - @amt where id = @debtor;
	`;
	let editExpenseData = [req.body.expenseID, req.body.amount, req.body.spender, req.body.debtor, req.body.tag, req.body.comment, req.body.date];

	// console.log(req.body);

	connection.query(editExpenseSQL, editExpenseData, (error, results, fields) => { 
		if (error) {
			console.log(error.message);
		}
